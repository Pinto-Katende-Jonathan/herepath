"""Tests for the herepath command-line interface."""

import pytest

from herepath import __main__ as cli
...
def test_cli_reports_error_cleanly(tmp_path, capsys, monkeypatch):
    # A misconfigured HEREPATH_ROOT must yield a clean error + exit 1, not a
    # traceback (so `ROOT="$(herepath)"` fails predictably).
