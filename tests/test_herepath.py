...
def reset_state(monkeypatch, tmp_path):
    ...
    herepath.reset_criteria()  # also clears the cached root
    yield
    herepath.reset_criteria()
...
def test_here_finds_pyproject_root(tmp_path):
    ...
    os.chdir(sub)
    assert herepath.here() == tmp_path.resolve()
    assert herepath.here("data", "x.csv") == (tmp_path / "data" / "x.csv").resolve()


def test_here_joins_slash_components(tmp_path):
    _touch(tmp_path / ".here")
    assert herepath.here("data/sub/x.csv") == (tmp_path / "data" / "sub" / "x.csv").resolve()


def test_here_returns_absolute_paths_unchanged(tmp_path):
    _touch(tmp_path / ".here")
    data = herepath.here("data")  # absolute
    assert data.is_absolute()
    # an absolute single arg comes back unchanged
    assert herepath.here(data) == data
    # an absolute anchor is kept, with further components appended
    assert herepath.here(data, "penguins.csv") == data / "penguins.csv"
...
def test_symlinked_subdir_not_resolved_in_result(tmp_path):
    ...
    assert herepath.here("data") == tmp_path.resolve() / "data"
    assert herepath.here("data") != external.resolve()
...
def test_i_am_uuid_match(tmp_path):
    ...
    root = herepath.i_am("scripts/run.py", uuid="abc-123-unique")
    assert root == tmp_path.resolve()
...
def test_i_am_uuid_mismatch_raises(tmp_path):
    ...
    with pytest.raises(FileNotFoundError):
        herepath.i_am("scripts/run.py", uuid="missing-id")
...
def test_env_var_overrides_detection(tmp_path, monkeypatch):
    ...
    assert herepath.here() == forced.resolve()
...
def test_reset_redetects_root(tmp_path):
    ...
    _touch(deep / ".here")
    os.chdir(deep)
    assert herepath.here() == tmp_path.resolve()  # still cached
    herepath.reset()
    assert herepath.here() == deep.resolve()  # re-detected
...
def test_find_root_custom_criterion(tmp_path):
    ...
    deep = tmp_path / "a" / "b"
    deep.mkdir(parents=True)
    root = herepath.find_root(herepath.has_file("Makefile"), start=deep)
    assert root == tmp_path.resolve()


def test_find_root_any_of_multiple(tmp_path):
    (tmp_path / ".git").mkdir()
    root = herepath.find_root(
        herepath.has_file("Makefile"), herepath.has_dir(".git"), start=tmp_path
    )
    assert root == tmp_path.resolve()
...
def test_find_root_glob(tmp_path):
    ...
    root = herepath.find_root(herepath.has_glob("*.toml"), start=tmp_path)
    assert root == tmp_path.resolve()


def test_find_root_not_found_raises(tmp_path):
    with pytest.raises(FileNotFoundError):
        herepath.find_root(herepath.has_file("definitely-absent.xyz"), start=tmp_path)


def test_find_root_does_not_affect_session(tmp_path):
    _touch(tmp_path / ".here")
    herepath.find_root(herepath.has_file(".here"), start=tmp_path)
...
def test_set_criteria_custom_marker(tmp_path):
    ...
    sub.mkdir()
    os.chdir(sub)
    herepath.set_criteria(herepath.has_file("company_project.json"))
    assert herepath.here() == tmp_path.resolve()
...
def test_set_criteria_replaces_defaults(tmp_path):
    ...
    _touch(tmp_path / ".here")
    herepath.set_criteria(herepath.has_file("never-present.marker"))
...
def test_set_criteria_requires_argument():
    with pytest.raises(ValueError):
        herepath.set_criteria()


def test_reset_criteria_restores_defaults(tmp_path):
    _touch(tmp_path / ".here")
    herepath.set_criteria(herepath.has_file("nope.marker"))
    herepath.reset_criteria()
    assert herepath.here() == tmp_path.resolve()
...
def test_using_root_temporary_override(tmp_path):
    ...
    other.mkdir()
    with herepath.using_root(other) as root:
        assert root == other.resolve()
        assert herepath.here("data") == (other / "data").resolve()
...
def test_using_root_restores_uninitialised_state(tmp_path):
    other = tmp_path / "other"
    other.mkdir()
    with herepath.using_root(other):
        assert herepath.here() == other.resolve()
